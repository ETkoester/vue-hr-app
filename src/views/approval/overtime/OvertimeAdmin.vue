        ...
        <tr class="text-center">
          ...
          <td>
            <div class="hidden md:inline-flex">
              <v-tooltip location="top">
                <template #activator="{ props }">
                  <v-btn v-if="status == 'pending'" density="compact" color="green-darken-4" icon="mdi-check" size="large" class="mr-2" v-bind="props" @click="onAction(item.raw.id, 'approve')" />
                </template>
                <span>批准</span>
              </v-tooltip>
              <v-tooltip location="top">
                <template #activator="{ props }">
                  <v-btn v-if="status == 'pending'" density="compact" color="red" icon="mdi-close" size="large" class="mr-2" v-bind="props" @click="onAction(item.raw.id, 'reject')" />
                </template>
                <span>否決</span>
              </v-tooltip>
              <v-tooltip location="top">
            ...
            <v-menu>
              ...
              <v-list>
                <v-list-item v-if="status == 'pending'" @click="onAction(item.raw.id, 'approve')">
                  <v-list-item-title class="flex items-center"> <v-icon icon="mdi-check" /> 批准 </v-list-item-title>
                </v-list-item>
                <v-list-item v-if="status == 'pending'" @click="onAction(item.raw.id, 'reject')">
...
<script setup lang="ts">
  ...
  interface Overtime extends ApprovalDetail {
    id: string;
    name: string;
  }
  ...
  const search = ref('');
  const leaves = reactive<Overtime[]>([]);
  const filterLeaves = computed(() => leaves.filter((e) => e.status == status.value));

  onMounted(async () => {
    loadingStore.update(true);
    await getUsers();
    const OvertimeRecords = await get(dbRef('company/overtime'));
    getOvertimes(OvertimeRecords);
  ...
  const getOvertimes = (snapshot: DataSnapshot) => {
    leaves.length = 0;
    if (snapshot.val() == undefined) return;
    Object.entries(snapshot.val()).forEach(([id, value]) => {
      const leave = { name: '', id: id, ...(value as object) } as Overtime;
      leave.name = username[leave.empID];
      leaves.push(leave);
    });
  };

  const unsubscribe = onValue(dbRef('company/overtime'), (snapshot) => {
    getOvertimes(snapshot);
  });
  ...
  const onAction = async (id: string, action: string) => {
    ...
    switch (action) {
      case 'approve':
        await update(dbRef(`company/overtime/${id}`), { status: 'approved' });
        break;
      case 'reject':
        await update(dbRef(`company/overtime/${id}`), { status: 'rejected' });
        break;
      case 'delete':
        await remove(dbRef(`company/overtime/${id}`));
